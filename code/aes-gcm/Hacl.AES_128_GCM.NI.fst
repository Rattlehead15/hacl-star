...
let aes128_gcm_compute_iv ctx iv_len iv =
  ...
  if (iv_len =. size 12) then (
    ...
    let aes_ctx = sub ctx (size 0) (size 12) in
    aes128_set_nonce aes_ctx iv;
    aes128_key_block tag_mix aes_ctx (u32 1);
    ctx.(17ul) <- vec_load_le U128 1 tag_mix;
    pop_frame();
    u32 2)
  else (
    ...
    let tag_mix = create 16ul (u8 0) in
    let aes_ctx = sub ctx (size 0) (size 12) in
    let gcm_ctx = sub ctx (size 12) (size 5) in
...
val aes128_gcm_malloc:
    r:rid
  -> ST.ST (s:buffer uint128 { length s = UInt32.v aes_gcm_ctx_len })
  (requires (fun _ ->
...
let aes128_gcm_malloc r =
  malloc r (u128 0) aes_gcm_ctx_len

val aes128_gcm_free:
    s:buffer uint128 { length s = UInt32.v aes_gcm_ctx_len }
  -> ST.ST unit
  (requires fun h0 ->
