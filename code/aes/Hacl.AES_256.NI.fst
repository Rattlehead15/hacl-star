...
[@ CInline ]
let aes256_key_block kb ctx = aes_key_block #MAES #Spec.AES.AES256 kb ctx
...
let aes256_ctr len out inp ctx =  aes_ctr #MAES #Spec.AES.AES256 len out inp ctx
...
let aes256_ctr_encrypt len out inp k n = aes_ctr_encrypt #MAES #Spec.AES.AES256 len out inp k n
...
val aes256_ctr_decrypt:
  ...
  (requires (fun h -> live h out /\ live h inp /\ live h k /\ live h n))
  (ensures (fun h0 _ h1 -> modifies (loc out) h0 h1))
let aes256_ctr_decrypt len out inp k n = aes_ctr_decrypt #MAES #Spec.AES.AES256 len out inp k n
